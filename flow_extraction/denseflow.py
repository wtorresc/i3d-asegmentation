...
def dense_flow(augs):
    ...
    try:
        videocapture = [d.name for d in os.scandir(video_path) if d.is_file()]
        videocapture.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))
    ...
    while True:
        ...
        frame = cv2.imread(os.path.join(video_path, videocapture[num0] ))[:, :, [2, 1, 0]]# (BGR -> RGB)
        num0+=1
        if frame_num==0:
...
def get_video_list():
    ...
    try:
        list_processed = [d.name for d in os.scandir(flow_saving_dir) if d.is_file()]
    except:
        list_processed = []
